...
class TransparentFaceRecognizer:
    ...
    def create_new_face_data(self, img, face, shape, face_feature):
        """为陌生人脸创建新的数据"""
        # 生成随机名字 (格式: Unknown_随机字符串_时间戳)
        random_str = ''.join(random.choices(string.ascii_letters + string.digits, k=4))
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        new_name = f"Unknown_{random_str}_{timestamp}"
        
        # 创建人脸文件夹
        path_photos_from_camera = "data/data_faces_from_camera/"
        new_face_dir = os.path.join(path_photos_from_camera, f"person_{new_name}")
        os.makedirs(new_face_dir, exist_ok=True)
        
        # 保存人脸图片
        face_img = img[face.top():face.bottom(), face.left():face.right()]
        face_img_path = os.path.join(new_face_dir, f"img_face_1.jpg")
        cv2.imwrite(face_img_path, cv2.cvtColor(face_img, cv2.COLOR_RGB2BGR))
        
        # 更新人脸数据库
        self.face_name_known_list.append(new_name)
        self.face_feature_known_list.append(face_feature)
        self.face_image_path_list.append(face_img_path)
        
        # 更新CSV文件
        self.update_face_database_csv(new_name, face_feature)
        
        # 更新当前帧的人脸名称
        idx = len(self.current_frame_face_name_list) - 1
        self.current_frame_face_name_list[idx] = new_name
        self.current_frame_face_known_list[idx] = True
        
        logging.info(f"Created new face data for: {new_name}")
        ...
        if faces:
            ...
            for i, face in enumerate(faces):
                ...
                if min_dist < 0.6:  # 阈值 
                    ...
                    # 检测到陌生人脸，自动创建新的人脸数据
                    self.create_new_face_data(img, face, shape, self.current_frame_face_feature_list[i])
